...
// Si ce script n'est pas appelé depuis la page de confirmation on construit la page panier.
    ...
    let cName, cPName, cMail, cAdd, cCp;
    cName = cPName = cMail = cAdd = cCp = 0;
    let products = [];
    let contact = {};
    ...
    champCp.addEventListener("input", () => {
        const errorCp = document.getElementById("cityErrorMsg");
        if (champCp.value.length == 5) {
            if (!verifCP(champCp.value)) {
                champCp.style.borderColor = "2px solid red";
                errorCp.innerText = "Veuillez saisir un code postal valide !";
                cCp = 1;
            } else {
                champCp.style.border = "0";
                errorCp.innerText = "";
                cCp = 0;
                getVille(champCp.value);
            }
        }
    });
    ...
    champMail.addEventListener("change", () => {
        ...
        if (!verifMail(champMail.value)) {
            champMail.style.borderColor = "2px solid red";
    ...
    subButton.addEventListener('click', (e) => {
        ...
        if (cName == 0 && cPName == 0 && cMail == 0 && cAdd == 0 && cCp == 0 && select.value != "") {
            verificationFinale();
        } else {
    ...
    function verificationFinale() {
        ...
        // Si le panier n'a pas été vidé on envoie la liste des id dans l'array 'products'
            ...
            let cityFull = champCp.value + " " + select.value;
            contact.city = cityFull;
    ...
    // 
    /**
     * Vérifie que le code postal saisi ne comporte que 5 chiffres.
     * @param { String } v 
     * @returns booléen
     */
    function verifCP(v) {
    ...
    /**
     * Cette fonction envoie le code postal saisi à l'API du gouvernement français pour récupérer le nom de la ville associé.
     * @param { String } cp Code postal dont on cherche le nom de ville associé.
     */
    function getVille(cp) {
        // On supprime tous les éléments option (sauf le 1er) du select au cas où une recherche aurait été faite précédemment.
